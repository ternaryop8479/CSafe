/*
 * SHA256.h
 * CSafe杀毒引擎中获取文件SHA-256的头文件定义和封装
 * 来自GitHub上的hash-library哈希库
 *  |
 *  V
*/
...
//这个项目用的zlib协议，允许闭源并作为收费商业软件使用，
//但是我们还是选择开放源代码

//源文件见下(这里是直接把注释啥的照抄下来了，除了sha256.cpp的文件头部删掉了
//(包括版权，不过两段头除了文件名不一样其他都一样，而且删不删这段代码都是人家的)，
//还加了一个calculate_file_sha256函数，其他地方一个字没动)
...
std::string calculate_file_sha256(const std::string &path) {
	...
	std::ifstream file(path, std::ios::binary);//打开目标文件

	if (!file && (!path.empty() || path != "\0")) {//如果文件无法成功打开并且输入不是无效值
		return path;//返回文件名
	}

	if(path.empty() || path == "\0") {
		return "(Cannot calculate file SHA-256)";
	}

	Filess << file.rdbuf();

	return sha256(Filess.str());
}